...
async function seed() {
  ...
  for (const [number, name, ayahCount] of SURAHS) {
    await sql`
      INSERT INTO surahs (number, name, ayah_count) VALUES (${number}, ${name}, ${ayahCount})
  ...
  // Seed default admin
  const hashed = await hashSHA256('admin123')
  await sql`
    INSERT INTO users (role, username, password, full_name, status)
    VALUES ('admin', 'admin', ${hashed}, 'مدير النظام', 'active')
